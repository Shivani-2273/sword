    ...
    public class GuestTokenResource {
        ...
        public String getGuestToken(@Context HttpServletRequest httpServletRequest) {
            try {
                ...
                OAuth2Application app = _oAuth2ApplicationLocalService
                        ...
                        .stream()
                        .filter(a -> "Guest Service".equals(a.getName()))
                        .findFirst()
                        .orElseThrow(() -> new Exception("OAuth2 application not found"));

                String tokenUrl = _portal.getPortalURL(httpServletRequest) + "/o/oauth2/token";
                String token = getOAuthToken(app.getClientId(), app.getClientSecret(),tokenUrl);
                return "{\"token\":\"" + token + "\"}";
